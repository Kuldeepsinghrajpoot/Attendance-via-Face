...
export default function CameraCapture({ attendanceData }: any) {
  ...
  const captureAndVerify = async () => {
    if (videoRef.current && canvasRef.current) {
      ...
      if (context) {
        ...
        try {
          const verifyResponse = await fetch("http://127.0.0.1:5000/verify-face", {
            method: "POST",
  ...
  const markAttendance = async (name: string) => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_PORT}/api/attendance?id=${id}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="flex flex-col items-center gap-4 p-4">
        <div className="flex flex-col items-center gap-4">
          {!capturedImage && <video ref={videoRef} autoPlay playsInline className=" w-80 h-60 border rounded-lg " />}
